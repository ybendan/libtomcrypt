                     ...
                     ecc_key *key)
{
   ...
   if (err == CRYPT_OK) {
      ...
      if ((err = ecc_set_dp_oid(curveoid, alg_seq[1].size, key)) != CRYPT_OK) {
         goto LBL_ERR;
      }
   }
   else {
      ...
      if ((err = ecc_set_dp_bn(a, b, prime, order, gx, gy, cofactor, key)) != CRYPT_OK) { goto LBL_ERR; }
   }
   ...
   if ((err = ecc_set_key(bin_k, seq_priv[1].size, PK_PRIVATE, key)) != CRYPT_OK) { goto LBL_ERR; }
   /* success */
   return err;
