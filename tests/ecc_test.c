...
int _ecc_import_export(void) {
   ...
   /* the following test keys were generated by these commands:
   ...
   DO(ecc_set_dp(dp, &key)); DO(ecc_set_key(raw_pri,  sizeof(raw_pri),  PK_PRIVATE, &key)); ecc_free(&key);
   DO(ecc_set_dp(dp, &key)); DO(ecc_set_key(raw_pub,  sizeof(raw_pub),  PK_PUBLIC,  &key)); ecc_free(&key);
   DO(ecc_set_dp(dp, &key)); DO(ecc_set_key(raw_pubc, sizeof(raw_pubc), PK_PUBLIC,  &key)); ecc_free(&key);

   /* import - openssl compatible DER format */
   DO(ecc_import_openssl(long_pri,   sizeof(long_pri),   &key)); ecc_free(&key);
   DO(ecc_import_openssl(long_pric,  sizeof(long_pric),  &key)); ecc_free(&key);
   DO(ecc_import_openssl(long_pub,   sizeof(long_pub),   &key)); ecc_free(&key);
   DO(ecc_import_openssl(long_pubc,  sizeof(long_pubc),  &key)); ecc_free(&key);
   DO(ecc_import_openssl(short_pri,  sizeof(short_pri),  &key)); ecc_free(&key);
   DO(ecc_import_openssl(short_pric, sizeof(short_pric), &key)); ecc_free(&key);
   DO(ecc_import_openssl(short_pub,  sizeof(short_pub),  &key)); ecc_free(&key);
   DO(ecc_import_openssl(short_pubc, sizeof(short_pubc), &key)); ecc_free(&key);

   /* import - PKCS8 format */
   DO(ecc_import_pkcs8(long_pri_pkcs8,   sizeof(long_pri_pkcs8),   NULL, 0, &key)); ecc_free(&key);
   DO(ecc_import_pkcs8(long_pric_pkcs8,  sizeof(long_pric_pkcs8),  NULL, 0, &key)); ecc_free(&key);
   DO(ecc_import_pkcs8(short_pri_pkcs8,  sizeof(short_pri_pkcs8),  NULL, 0, &key)); ecc_free(&key);
   DO(ecc_import_pkcs8(short_pric_pkcs8, sizeof(short_pric_pkcs8), NULL, 0, &key)); ecc_free(&key);

   DO(ecc_import_openssl(long_pri, sizeof(long_pri), &key));
   /* export - openssl compatible DER format */
   outlen = sizeof(out); DO(ecc_export_openssl(out, &outlen, PK_PRIVATE, &key));
   outlen = sizeof(out); DO(ecc_export_openssl(out, &outlen, PK_PUBLIC,  &key));
   outlen = sizeof(out); DO(ecc_export_openssl(out, &outlen, PK_PRIVATE|PK_CURVEOID, &key));
   outlen = sizeof(out); DO(ecc_export_openssl(out, &outlen, PK_PUBLIC|PK_CURVEOID,  &key));
   /* export - raw keys */
   outlen = sizeof(out); DO(ecc_get_key(out, &outlen, PK_PRIVATE, &key));
   outlen = sizeof(out); DO(ecc_get_key(out, &outlen, PK_PUBLIC, &key));
   outlen = sizeof(out); DO(ecc_get_key(out, &outlen, PK_PUBLIC|PK_COMPRESSED, &key));
   ecc_free(&key);
